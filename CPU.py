...
class CPU:
    def __init__(self, memory, system):
        ...
        self.memory_buffer = []  # Buffer for producer-consumer operations
        self.producer_id = 0  # Default producer ID
        self.consumer_id = 0  # Default consumer ID
    ...
    def _swi(self, operands, pcb):
        ...
        if swi == 1:  # End of file
            ...
            print(f"Producing message with ID: {self.producer_id}")
            Producer.produce(self.producer_id, self.memory_buffer)

        elif swi == 31:  # CONSUME
            print(f"Consuming message with ID: {self.consumer_id}")
            Consumer.consume(self.consumer_id, self.memory_buffer)
    ...
    def _div(self, operands):
        ...
        if self.registers[third_register] == 0:
            self.system_call(104)  # Division by zero error code
            self.registers[first_register] = 0  # Set result to 0
            if self.verbose:
                print("\tDIV\tDivision by zero error")
            return

        self.registers[first_register] = self.registers[second_register] // self.registers[third_register]
        if self.verbose:
            print(
                f"\tDIV\t{self.registers[first_register]} = {self.registers[second_register]} / {self.registers[third_register]}")
    ...
    def _mov(self, operands):
        ...
        if self.verbose:
            print(
                f"\tMOV\tR{first_register} <= R{second_register}\t\t\t{self.registers}")
    ...
    def _mvi(self, operands):
        ...
        if self.verbose:
            print(
                f"\tMVI\tR{register} <= {immediate_value}\t\t\t{self.registers}")
    ...
    def _beq(self, operands):
        ...
        """
        address_bytes = operands[0:4]
        address = struct.unpack('<I', bytes(address_bytes))[
            0] + self.pcb.loader
    ...
    def _orr(self, operands):
        ...
        if self.verbose:
            print(
                f" - ORR {self.registers[self.z]} = {self.registers[first_register]} ({second_register}) | {self.registers[second_register]} ({second_register})")
    ...
    def _eor(self, operands):
        ...
        if self.verbose:
            print(
                f" - EOR {self.registers[self.z]} = {self.registers[first_register]} ({second_register}) ^ {self.registers[second_register]} ({second_register})")
