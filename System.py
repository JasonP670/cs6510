...
class System:
    ...
    def execute(self, *args):
        ...
        for i in range(0, len(args), 2):
            ...
            if filepath.endswith('producer.osx'):
                producer_id = int(arrival_time)
                print(f"Producing message with ID: {producer_id}")
                Producer.produce(
                    producer_id, self.memory_buffer)
            elif filepath.endswith('consumer.osx'):
                consumer_id = int(arrival_time)
                print(f"Consuming message with ID: {consumer_id}")
                Consumer.consume(
                    consumer_id, self.memory_buffer)
            else:
